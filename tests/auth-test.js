 ...
 * Tests all authentication and integration functionality against MySQL backend
 * 
 ...
 * 
 * Run with: npm run test:auth or npm run test:mysql
 */
...
async function runTests() {
  ...
  // Test 15: Invalid Token Refresh with Malformed Token
  try {
    totalTests++;
    console.log('Test 15: Invalid Token Refresh with Malformed Token (Should Fail)');
    try {
      await axios.post(`${BASE_URL}/api/refresh`, {
  ...
  // Test 16: Token Structure and Expiry Validation
  try {
    totalTests++;
    console.log('Test 16: Token Structure and Expiry Validation');
    ...
    console.log('✅ PASS - Token structure and expiry validated\n');
    passedTests++;
  } catch (error) {
    console.log(`❌ FAIL - ${error.message}\n`);
    failedTests++;
  ...
  // Test 17: Login with Missing Credentials
  try {
    totalTests++;
    console.log('Test 17: Login with Missing Credentials (Should Fail)');
    let testsPassed = 0;
    let testsNeeded = 2;
    ...
    if (testsPassed === testsNeeded) {
      console.log('✅ PASS - Missing credentials correctly rejected\n');
      passedTests++;
    } else {
      console.log(`❌ FAIL - Only ${testsPassed}/${testsNeeded} missing credential checks passed\n`);
      failedTests++;
    }
  } catch (error) {
    console.log(`❌ FAIL - ${error.message}\n`);
  ...
  // Test 18: Login with Non-existent User
  try {
    totalTests++;
    console.log('Test 18: Login with Non-existent User (Should Fail)');
    try {
      await axios.post(`${BASE_URL}/api/login`, {
        email: 'nonexistent@example.com',
        password: 'SomePassword123!'
      }, testConfig);
      console.log('❌ FAIL - Should reject non-existent user\n');
      failedTests++;
    } catch (error) {
      // Accept 401 (unauthorized) or 423 (rate limited) as both indicate rejection
