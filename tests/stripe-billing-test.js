...
class StripeBillingTestRunner {
  ...
  async testSoftDeleteCancelsStripeAndWebhookSyncWhileDeleted() {
    ...
    try {
      ...
      // Stripe may deliver subscription.deleted after soft-delete. syncPremium must
      // not throw on deleted users (previously returned 400 forever).
      ...
      // Login restores the soft-deleted account; premium must stay false (no active sub).
