...
class StripeBillingTestRunner {
  ...
  /**
   * Stale customer.subscription.updated after deleted must not resurrect premium.
   * Trigger: cancel via deleted webhook, then replay an older updated(active) event.
   */
  async testStaleSubscriptionUpdateDoesNotResurrectPremium() {
    ...
    try {
      ...
      // Delayed/retried update that still claims active — must be ignored.
